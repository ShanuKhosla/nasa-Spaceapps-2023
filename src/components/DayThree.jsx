...
import { daythrees } from "../constants";

const EventCard = ({ daythree }) => (
  <VerticalTimelineElement
    ...
    <ul className=" list-none max-w-full text-md md:text-xl font-overpass font-normal text-black">
      {daythree.points.map((point, index) => (
        <li
...
const DayThree = () => {
  ...
  return (
    ...
    <VerticalTimeline>
        <div></div>
          {daythrees.map((daythree, index) => (
            <EventCard key={index} daythree={daythree} />
          ))}
        </VerticalTimeline>
      </motion.div>
    </motion.div>
